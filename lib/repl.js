      ...
      try {
        ...
        try {
          ...
          try {
            ...
            ret = vm.runInContext(
              __oni_rt.c1.compile('(' + self.bufferedCommand + ')',
                                  {filename:"repl"}),
              context,
              'repl');
  ...
  if (match) {
    ...
    if (!subdir) {
      ...
      var builtinLibs = ['assert', 'buffer', 'child_process', 'crypto', 'dgram',
        ...
        'querystring', 'readline', 'repl', 'string_decoder', 'util', 'url',
        '$child_process', '$cutil', '$dns', '$fs', '$http', '$net', '$stream' ];
      ...
      if (!expr) {
        ...
        if (filter) {
          completionGroups.push(['break', 'case', 'catch', 'const',
            ...
            'while', 'with', 'yield',
            'waitfor', 'and', 'or', 'spawn', 'hold', 'using' ]);
        }
      } else {
        try {
          obj = vm.runInContext(__oni_rt.c1.compile(expr, {filename:'repl'}), this.context, 'repl');
        } catch (e) {
